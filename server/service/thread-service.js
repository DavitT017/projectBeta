...
async function getAllThreadsWithComments(req, res) {
    try {
        const query = `
            SELECT thread_id, title, description, user_id, created_at FROM "threads";
        `;
...
async function getThreadWithComments(req, res) {
    try {
        const threadQuery = `
            SELECT thread_id, title, description, user_id, created_at
            FROM "threads"
            WHERE thread_id = $1;
        `;
        const threadResult = await pool.query(threadQuery, [req.params.thread_id]);
        ...
        const commentsQuery = `
            ...
            ORDER BY c.createdat DESC;
        `;
        const commentsResult = await pool.query(commentsQuery, [req.params.thread_id]);
        const comments = commentsResult.rows;
