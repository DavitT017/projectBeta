...
class UserService {
    async registration(username, email, password) {
        try {
            ...
            if (rows.length > 0) {
                throw ApiError.BadRequest(`User with ${email} email or ${username} username already exists`);
            }

            const hashPassword = await bcrypt.hash(password, 10);
            const user_id = uuid.v4(); // or any other method to generate a unique id

            const createUserQuery = 'INSERT INTO Users (user_id, username, email, password_hash) VALUES ($1, $2, $3, $4) RETURNING *';
            const createUserValues = [user_id, username, email, hashPassword];
            const newUser = await client.query(createUserQuery, createUserValues);
    ...
    async activate(activationLink) {
        ...
        const updateUserQuery = 'UPDATE users SET is_activated = true WHERE id = $1';
