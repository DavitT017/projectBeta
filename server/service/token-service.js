...
class TokenService {
    async saveToken(userId, refreshToken) {
        ...
        try {
            ...
            const { rows } = await client.query(checkTokenQuery, checkTokenValues);
            if (rows.length > 0) {
                const updateTokenQuery = 'UPDATE tokens SET refresh_token = $1 WHERE user_id = $2 RETURNING *';
            ...
            const insertTokenQuery = 'INSERT INTO tokens (user_id, refresh_token) VALUES ($1, $2) RETURNING *';
            const insertTokenValues = [userId, refreshToken];
            const newToken = await client.query(insertTokenQuery, insertTokenValues);
    ...
    async removeToken(refreshToken) {
        ...
        try {
            ...
            const deleteTokenQuery = 'DELETE FROM tokens WHERE refresh_token = $1';
    ...
    async findToken(refreshToken) {
        const client = await pool.connect();
        try {
            const findTokenQuery = 'SELECT * FROM tokens WHERE refresh_token = $1';
